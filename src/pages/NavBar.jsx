  ...
  const fetchUserData = async () => {
    auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        // 🔹 Check Microsoft Login if no Firebase user
        const accounts = msalInstance.getAllAccounts();
        if (accounts.length > 0) {
          const activeAccount = accounts[0];
          msalInstance.setActiveAccount(activeAccount);
          console.log("✅ Microsoft User:", activeAccount);

          setUserDetails(activeAccount); // Directly setting the active Microsoft account
        } else {
          setUserDetails(null); // No user found
        }
      }
    });
  };
  ...
  const navItems = [
    {
      key: "yourWork",
      text: "Your Work",
      subItems: ["Recent Work", "Starred", "Assigned to You"],
    },
    {
      key: "projects",
      text: "Projects",
      subItems: ["All Projects", "Favorites", "Archived"],
    },
    {
      key: "filters",
      text: "Filters",
      subItems: ["My Filters", "Create Filter"],
    },
    {
      key: "dashboards",
      text: "Dashboards",
      subItems: ["My Dashboards", "Shared Dashboards"],
    },
